...
export type Locale = "en" | "zh-TW" | "pt";
export type MessageKey = keyof typeof zhTW;

const dictionaries: Record<Exclude<Locale, "en">, Record<MessageKey, string>> = {
  "zh-TW": zhTW,
  pt,
};

export function isLocale(value: unknown): value is Locale {
  return value === "en" || value === "zh-TW" || value === "pt";
}
...
export function createI18n(locale: Locale) {
  function t<K extends MessageKey>(key: K, ...args: MessageArgs<K>): string {
    const message = locale === "en" ? key : dictionaries[locale][key];
