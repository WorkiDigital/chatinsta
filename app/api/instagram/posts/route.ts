...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      // Codes, not display text: this route has no user locale to translate
      // into, so the client maps each code to a translated string.
      limitationCodes: account.provider === "ZERNIO" ? ["zernio_recent_posts_limit"] : [],
    });
  } catch (err) {
