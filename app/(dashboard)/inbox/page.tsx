...
// Instagram's Conversations API returns no profile photo on the list
// endpoint (Meta or Zernio) — fetching one per conversation would mean 50+
// extra rate-limited calls just to paint the list. A stable per-username
// gradient initial stands in instead.
const AVATAR_GRADIENTS = [
  "linear-gradient(135deg, #f9622e, #f43f7d)",
...
function Avatar({ username, size = 36 }: { username: string | null; size?: number }) {
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <Avatar username={c.contact.username} />
          ...
          {!active ? (
            ...
            <>
              <div className="flex shrink-0 items-center gap-2 border-b border-border px-4 py-3 text-sm font-semibold text-foreground">
                ...
                <Avatar username={active.contact.username} size={28} />
                  ...
                  messages.map((m) => (
                    ...
                    >
                      <div
                        ...
                        // A gradient repeated on every bubble in a long thread reads as
                        // noise; reserve it for one-off CTAs and use the flat accent here.
                        style={m.fromMe ? { background: "var(--color-accent)" } : undefined}
                      >
